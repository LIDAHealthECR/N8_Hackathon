...
# Download Data -----------------------------------------------------------
RA_raw <- read.csv("Data/Rheumatoid Arthritis dataset before normalization.csv")

# Batch correct -----------------------------------------------------------

# Set gene names as rownames
rownames(RA_raw) <- RA_raw$geneNames
...
batch_colors <- c(
  "GSE12021" = "#E41A1C",  # red
  "GSE55457" = "#377EB8",  # blue
  "GSE77298" = "#FF7F00"   # orange
)
...
# Save last plot
ggsave("Figures/RA_PCA_batch_correction.png",
       plot = combined_plot,
       width = 15,
       height = 5,
       dpi = 300)
...
write.csv(edata_out,
          file = "Files/RA_batch_corrected.csv",
          row.names = FALSE)
